...
#     set -g btf_title_display_process no
...
function fish_title
  ...
  if [ "$btf_title_display_process" = 'yes' ]
    echo $_

    [ "$btf_title_display_path" != 'no' ]
      and echo ' '
  end

  if [ "$btf_title_display_path" != 'no' ]
    if [ "$btf_title_use_abbreviated_path" = 'no' ]
      echo $PWD
    else
      prompt_pwd
    end
